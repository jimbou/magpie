...
def patch_subdirectories():
    ...
    for entry in os.listdir():
        ...
        if os.path.isdir(f"{entry}/necessary"):
            ...
            subprocess.run('rm -rf triangle.py', shell=True, check=True)
            subprocess.run('cp ../../triangle.py .', shell=True, check=True)
            
            # Execute the patch command
            try:
                subprocess.run('patch triangle.py ../*.diff', shell=True, check=True)
